...
TEST(AnalyzeTypeTest, NullPointerTest) {
  ...
  EXPECT_NE(output.find("此区域不可访问"), std::string::npos);
}
...
// Test cases for mixed cv-qualifiers in multi-level pointer
TEST(AnalyzeTypeTest, MixedCvMultiLevelPointerTest1) {
  int value = 42;
  const int* ptr1 = &value;
...
TEST(AnalyzeTypeTest, MixedCvMultiLevelPointerTest2) {
  const volatile int a = 1;
  const volatile int* b = &a;
  const volatile int *const *c = &b;
  const volatile int *const **const d = &c;
  std::string output = captureOutput([&]() { analyzeType(d); });

  EXPECT_NE(output.find("数据类型: int const volatile* const** [ const ]"),
            std::string::npos);
  EXPECT_NE(output.find("层级: 3"), std::string::npos);
  EXPECT_NE(output.find("指向的数据类型: int const volatile* const*"),
            std::string::npos);
...
// Test case for r-value reference
TEST(AnalyzeTypeTest, RValueRefTest) {
  int value = 42;
  int&& rvalueRef = std::move(value);
  std::string output = captureOutput([&]() { analyzeType(rvalueRef); });

  EXPECT_NE(output.find("数据类型: int&&"), std::string::npos);
  EXPECT_NE(output.find("打印值: 42"), std::string::npos);
};
// Test case for const r-value reference
TEST(AnalyzeTypeTest, ConstRValueRefTest) {
  int value = 42;
  const int&& rvalueRef = std::move(value);
  std::string output = captureOutput([&]() { analyzeType(rvalueRef); });

  EXPECT_NE(output.find("数据类型: int&& [ const ]"), std::string::npos);
  EXPECT_NE(output.find("打印值: 42"), std::string::npos);
}
// Test case for literal r-value reference
TEST(AnalyzeTypeTest, LiteralRValueRefTest) {
  std::string output = captureOutput([&]() { analyzeType(nullptr); });

  EXPECT_NE(output.find("数据类型: std::nullptr_t&&"), std::string::npos);
  EXPECT_NE(output.find("打印值: nullptr"), std::string::npos);
}
